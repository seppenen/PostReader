  ...
  if (typeof data !== "undefined") {
    if (id) {
      return data.filter((post) => {
        return post.from_id.includes(id);
      });
    } else if (content) {
      return data.filter((post) => {
        return post.message.toLowerCase().includes(content.toLowerCase());
      });
    } else if (userName) {
      return data.filter((post) => {
        return post.from_name.toLowerCase().includes(userName.toLowerCase());
      });
...
export const distinct = (obj) => {
  ...
  return obj.filter(
    (value, index, self) =>
      index === self.findIndex((row) => row.from_name === value.from_name)
  );
};
// Sort object A-Z Z-A
